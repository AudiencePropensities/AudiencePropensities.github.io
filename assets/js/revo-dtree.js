! function(name, context, definition) {
  ...
  /***************************************************************************/
	...
	var resize = function(treeEl) {
		treeEl = treeEl || $('#tree'),
		container = treeEl.parent(),
		width = container.width() - (d3.select('#legend-container').classed('hide') ? 0 : 200);
	...
	var toggleLabels = function() {
		...
		d3.select("#body").selectAll(".tree-label").classed('hide', !visibility.labels);
	};

	function toggleLabelLength() {
		var len = visibility.labelLength;
		var els = d3.select("#body").selectAll(".tree-label")[0];
	...
	var toggleBorders = function() {
		...
		d3.select("#body").selectAll(".tree-node").style("stroke-width", function(d) {
	...
	var toggleColor = function() {
		...
		d3.select("#body").selectAll(".tree-node").style("fill", function(d) {
			var lmode = $('#3');
	...
	var toggleLegendMode = function(to, from) {
	...
	var toggleLegendArea = function() {
		$('#3').prop('disabled', visibility.area);
		$('#4').prop('disabled', visibility.area);
	...
	function loadDataset(json, p) {
	  ...
	  color_map = d3.scale.category10();
		...
		var colorRange = p ? p.range : Legend.palette.range;
	...
	function update(source) {
		...
		link.enter().insert("svg:path", "g")
			...
			return link_stoke_scale(d.target.n);
		})
	...
	function predictionPath(pD) {
		d3.select("#body").selectAll(".link").style("stroke", 'lightgray');
		...
		var paths = d3.select("#body").selectAll(".link")[0];
  ...
  /***************************************************************************/
	...
	DTree.prototype = {
		...
		bindUI: function() {
			...
			// -- Node Label Length --
			$('#legth').val('');
			d3.select('#label-length').on('keyup', function(e) {
